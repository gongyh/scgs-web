...
class RunProjPipeline implements ShouldQueue
{
	...
	$nextflow_profile = $pipeline_params->nextflow_profile;
        ...
        switch ($nextflow_profile) {
            case "Local":
                $profile_string = " -profile docker,base";
                break;
            case "Kubernetes":
        ...
        $command = $nextflow_path . ' run '. $nf_core_scgs_path . ' ' . $current_job->command . $profile_string . ' -name uuid-' . $current_job->current_uuid . ' -with-weblog '. env('WEBLOG_SERVER', 'http://localhost') .'/execute/start';
